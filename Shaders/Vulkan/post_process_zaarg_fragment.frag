...
void main() {
  ...
  float lum = getLuminance(screen_color);
  vec3 final_color = screen_color;
  vec3 shadow_color = vec3(push.shadowTintX, push.shadowTintY, push.shadowTintZ);

  float gray = rgbToGrayscale(final_color);
  if (lum < push.shadowThreshold) {
    final_color = mix(vec3(gray), shadow_color, push.shadowBlend);
    // final_color = vec3(push.shadowTintX, push.shadowTintY, push.shadowTintZ);
  }
  else if (lum < push.highlightThreshold) {
    // Midtones
    final_color = mix(vec3(gray), screen_color, push.blendFactor);
    // final_color = vec3(push.midtoneTintX, push.midtoneTintY, push.midtoneTintZ);
  } else {
    vec3 highlightTint = vec3(push.highlightTintX, push.highlightTintY, push.highlightTintZ);
    final_color = mix(screen_color, highlightTint, push.highlightBlend);
  }

  // float gray = rgbToGrayscale(screen_color);
  // vec3 redTintedColor = mix(vec3(gray), screen_color, );
  ...
  float vignetteStrength = 0.3; // how strongly to tint
