...
void main() {
  ...
  edge = pow(edge, 0.6);
  vec3 mix_result = mix(screen_color, vec3(0.0), edge);
  vec3 posterized_color = posterize(mix_result, push.dither_levels);
  ...
  float lum = dot(dithered_color, vec3(0.2126,0.7152,0.0722));
  // float lum = dot(dithered_color, vec3(push.luminanceX, push.luminanceY, push.luminanceZ));
  float flip = 0;
  if(push.blendFactor > 0) {
    flip = 1;
  }
  vec3 final_color = texture(sampler2D(_hatchTexture1, _hatchSampler1), vec2(abs(flip - lum), 0)).rgb;
