...
public static partial class GLTFLoaderKHR {
  ...
  private static void LoadAnimations(Gltf gltf, byte[] globalBuffer, MeshRenderer meshRenderer) {
    foreach (var anim in gltf.Animations) {
      var animation = new Neko.Rendering.Renderer3D.Animations.Animation();
      ...
      foreach (var samp in anim.Samplers) {
        var sampler = new Neko.Rendering.Renderer3D.Animations.AnimationSampler();

        if (samp.Interpolation == glTFLoader.Schema.AnimationSampler.InterpolationEnum.LINEAR) {
          sampler.Interpolation = Neko.Rendering.Renderer3D.Animations.AnimationSampler.InterpolationType.Linear;
        } else if (samp.Interpolation == glTFLoader.Schema.AnimationSampler.InterpolationEnum.STEP) {
          sampler.Interpolation = Neko.Rendering.Renderer3D.Animations.AnimationSampler.InterpolationType.Step;
        } else if (samp.Interpolation == glTFLoader.Schema.AnimationSampler.InterpolationEnum.CUBICSPLINE) {
          sampler.Interpolation = Neko.Rendering.Renderer3D.Animations.AnimationSampler.InterpolationType.CubicSpline;
        }
      ...
      foreach (var source in anim.Channels) {
        var channel = new Neko.Rendering.Renderer3D.Animations.AnimationChannel();

        if (source.Target.Path == AnimationChannelTarget.PathEnum.rotation) {
          channel.Path = Neko.Rendering.Renderer3D.Animations.PathType.Rotation;
        }
        if (source.Target.Path == AnimationChannelTarget.PathEnum.translation) {
          channel.Path = Neko.Rendering.Renderer3D.Animations.PathType.Translation;
        }
        if (source.Target.Path == AnimationChannelTarget.PathEnum.scale) {
          channel.Path = Neko.Rendering.Renderer3D.Animations.PathType.Scale;
        }
  ...
  private static void LoadSkins(
    ...
    out List<(Guid, Neko.Rendering.Renderer3D.Animations.Skin)> skins
  ) {
    skins = [];
    if (gltf.Skins == null) return;

    foreach (var source in gltf.Skins) {
      var newSkin = new Neko.Rendering.Renderer3D.Animations.Skin();
  ...
  private static void LoadNode(
    ...
    Neko.Rendering.Renderer3D.Node newNode = new(app, meshRenderer.Owner.Id) {
      Index = nodeIdx,
      Name = node.Name,
...
/*
  ...
  private unsafe static void LoadAnimationNode(
    ...
    nint allocator,
    IDevice device,
    Neko.Animations.AnimationNode parent,
    Node node,
    int nodeIdx,
    Gltf gltf,
    byte[] globalBuffer,
    ...
    Neko.Animations.AnimationNode newNode = new() {
      Index = nodeIdx,
      Name = node.Name,
      SkinIndex = node.Skin.HasValue ? node.Skin.Value : -1,
      NodeMatrix = Matrix4x4.Identity
