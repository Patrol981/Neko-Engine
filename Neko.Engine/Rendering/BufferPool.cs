...
public class BufferPool : IDisposable {
  ...
  internal class BufferData {
    internal NekoBuffer VertexBuffer = null!;
  ...
  public unsafe bool CreateNewIndexBuffer(in uint index, nint data, ulong byteSize) {
    ...
    var indexBuffer = new NekoBuffer(
      _allocator,
      _device,
      _maxBufferSize,
  ...
  private unsafe bool AddToExistingIndex(in uint index, ref NekoBuffer buffer, in nint data, in ulong byteSize, in ulong prevSize, ref string reason) {
    var sumSize = prevSize + byteSize;
  ...
  private unsafe bool AddToExistingBufferWithRebuild(in uint index, ref NekoBuffer buffer, in nint data, in ulong byteSize, in ulong prevSize, ref string reason) {
    var sumSize = prevSize + byteSize;
    if (sumSize >= _maxBufferSize) {
      reason = $"sumSize of [{sumSize}] bytes is too large compared to maximum [{_maxBufferSize}] bytes";
      return false;
  ...
  private unsafe bool AddToExistingBuffer(ref NekoBuffer buffer, in nint data, in ulong byteSize, in ulong prevSize, ref string reason) {
    var sumSize = prevSize + byteSize;
    if (sumSize >= _maxBufferSize) {
      reason = $"sumSize of [{sumSize}] bytes is too large compared to maximum [{_maxBufferSize}] bytes";
      return false;
