...
public partial class ImGuiController : IDisposable {
  ...
  private NekoBuffer _vertexBuffer = default!;
  private NekoBuffer _indexBuffer = default!;
  ...
  public async Task<Task> Init(int width, int height, bool createBuffers = true) {
    ...
    var io = ImGui.GetIO();
    io.Fonts.ClearFonts();
    var nekoPath = NekoPath.AssemblyDirectory;
    SmallFont = io.Fonts.AddFontFromFileTTF($"{nekoPath}/Resources/fonts/DroidSans.ttf", 15);
    MediumFont = io.Fonts.AddFontFromFileTTF($"{nekoPath}/Resources/fonts/DroidSans.ttf", 20);
    LargeFont = io.Fonts.AddFontFromFileTTF($"{nekoPath}/Resources/fonts/DroidSans.ttf", 30);
